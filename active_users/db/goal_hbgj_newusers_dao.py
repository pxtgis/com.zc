...
class NewusersDaily(Mysql):
    ...
    def update_users(self,_users):
        if (_users == None):
            ...
            db_name = 'hbgj_newusers_daily'
            sql="update hbgj_newusers_daily set new_users=%s,new_users_ios=%s,new_users_android=%s,updatetime=now() where  DATE_FORMAT(str_to_date(s_day,'%%Y-%%m-%%d'),'%%Y%%m%%d')=DATE_FORMAT(str_to_date(%s,'%%Y-%%m-%%d'),'%%Y%%m%%d')"
            args = []
            for user in _users:
